...
export const PaymentMethodsScreen: React.FC = () => {
  const { createMandate, revokeMandate } = useMandate();
  ...
  const createPaymentMandate = async (methodType: string): Promise<string | undefined> => {
    try {
      ...
      const mandate = await createMandate({
        ...
        constraints: {
          ...defaultConstraints,
          allowedPaymentMethods: [methodType],
        },
      });
      return mandate.id;
    } catch (err) {
      console.warn('Failed to create payment mandate:', err);
      return undefined;
    }
  };
  ...
  const handleAddCard = async () => {
    ...
    const mandateId = await createPaymentMandate('card');
    ...
    if (mandateId) {
      Alert.alert('Success', `${brand} card ending in ${last4} added with payment mandate activated`);
    } else {
      Alert.alert('Card Added', `${brand} card ending in ${last4} saved. Payment mandate could not be created - you can set it up later in Mandate Management.`);
    }
  };
  ...
  const handleAddPayPal = async () => {
    ...
    const mandateId = await createPaymentMandate('paypal');
    ...
    if (mandateId) {
      Alert.alert('Success', 'PayPal account linked with payment mandate activated');
    } else {
      Alert.alert('PayPal Linked', 'PayPal account saved. Payment mandate could not be created.');
    }
  };
  ...
  const handleDelete = (id: string) => {
    ...
    Alert.alert(
      ...
      `Remove ${method?.label}?${method?.mandateId ? ' The associated payment mandate will also be revoked.' : ''}`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            // Revoke mandate if one exists
            if (method?.mandateId) {
              try {
                await revokeMandate(method.mandateId, 'Payment method removed');
              } catch (err) {
                console.warn('Failed to revoke mandate:', err);
              }
            }
    ...
    if (method.mandateId) {
      return (
        <View style={styles.mandateBadge}>
          <Text style={styles.mandateBadgeText}>Mandate Active</Text>
        </View>
      );
    }
    return (
      <View style={styles.noMandateBadge}>
        <Text style={styles.noMandateBadgeText}>No Mandate</Text>
      </View>
    );
  };
