...
export const adminMerchantController = {
  ...
  registerAgent: async (req: Request, res: Response) => {
    try {
      ...
      const { name, slug, description, agentId, agentName, apiEndpoint, capabilities, metadata } = req.body;
  ...
  updateAgent: async (req: Request, res: Response) => {
    try {
      ...
      const { name, description, capabilities, status, apiEndpoint } = req.body;
